...
public class ConfigUtils {
    ...
    public static Properties getProperties() {
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    String path = System.getProperty(Constants.DEFAULT_HUSKY_PROPERTIES);
                    if (path == null || path.length() == 0) {
                        path = System.getenv(Constants.DEFAULT_HUSKY_PROPERTIES);
                        if (path == null || path.length() == 0) {
                            path = Constants.DEFAULT_HUSKY_PROPERTIES;
                        }
                    }
    ...
    public static void main(String[] args) {
        Properties properties = ConfigUtils.loadProperties("husky.properties");
